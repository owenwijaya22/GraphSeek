...
import requests
import json
from helper.pipeline_retrieve import retrieve_documents
from helper.documents_handler import process_documents
...
def check_models():
    try:
        ...
        required_models = [MODEL, EMBEDDINGS_MODEL]
        
        missing_models = [model for model in required_models if model not in available_models]
        
        if missing_models:
            st.error(f"⚠ Required models not found: {', '.join(missing_models)}")
            for model in missing_models:
                st.code(f"ollama pull {model}", language="bash")
            st.stop()
            
        # Optional: Display available models info
        st.sidebar.expander("📚 Model Information", expanded=False).json(response.json())
